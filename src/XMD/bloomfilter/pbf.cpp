...
#include "XMD/bloomfilter/pbf.h"

#include <cstring>

namespace pbf {

void _PageBloomFilter::init(unsigned page_level, unsigned page_num,
...
template <unsigned N>
bool PageBloomFilter<N>::test(const uint8_t* data,
...
template <unsigned N>
V128X PageBloomFilter<N>::partial_test1(const uint8_t* data, unsigned len,
                                        PageBloomFilter<N>* this_pbf,
                                        size_t& idx) {
  V128X t;
  t.v = Hash(data, len);
  idx = PageHash(t) % this_pbf->m_page_num;
  return t;
}

template <unsigned N>
bool PageBloomFilter<N>::partial_test2(size_t idx, V128X t) const noexcept {
...
class BloomFilterImp : public BloomFilter {
 public:
  ...
  size_t virual_capacity(float fpr) const noexcept {
    return self()->virual_capacity(fpr);
  }
  unsigned way() const noexcept { return self()->way(); }
  bool test(const uint8_t* data, unsigned len) const noexcept {
    return self()->test(data, len);
  }
  bool set(const uint8_t* data, unsigned len) noexcept {
    return self()->set(data, len);
  }

  explicit BloomFilterImp(PageBloomFilter<N>&& bf) { *self() = std::move(bf); }

 private:
  const PageBloomFilter<N>* self() const noexcept {
    return reinterpret_cast<const PageBloomFilter<N>*>(
        static_cast<const _PageBloomFilter*>(this));
  }
  PageBloomFilter<N>* self() noexcept {
    return reinterpret_cast<PageBloomFilter<N>*>(
        static_cast<_PageBloomFilter*>(this));
  }
};
...
std::unique_ptr<BloomFilter> New(unsigned way, unsigned page_level,
                                 unsigned page_num, size_t unique_cnt,
                                 const uint8_t* data) {
#define PBF_NEW_CASE(w)                                             \
  case w: {                                                         \
