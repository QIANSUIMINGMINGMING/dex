#ifndef FADEBUCKET_CONTAINER_H
#define FADEBUCKET_CONTAINER_H
...
class bucket_container {
 ...
 public:
  ...
  class faded_bucket {
   public:
    ...
    bool newerTS(size_type ind, uint64_t ts) const { return TSs_[ind] < ts; }

    bool checkTS(size_type ind) const { return TSs_[ind] != 0; }
    uint64_t get_TS(size_type ind) const { return TSs_[ind]; }
  ...
  bucket_container(size_type hp, const allocator_type &allocator)
    ...
    // elements.
    static_assert(std::is_nothrow_constructible<faded_bucket>::value,
  ...
  faded_bucket &operator[](size_type i) { return buckets_[i]; }
  const faded_bucket &operator[](size_type i) const { return buckets_[i]; }
 ...
 private:
  using bucket_traits_ = typename traits_::template rebind_traits<faded_bucket>;
  ...
  void destroy_buckets() noexcept {
    ...
    // worry about dealing with exceptions when constructing all the
    // elements.
    static_assert(std::is_nothrow_destructible<faded_bucket>::value,
                  "bucket_container requires bucket to be nothrow "
                  "destructible");
    clear();
    for (size_type i = 0; i < size(); ++i) {
  ...
  typename traits_::template rebind_alloc<faded_bucket> bucket_allocator_;
  ...
  operator>>(std::istream &is, bucket_container<ThisKey, ThisT, Allocator,
    ...
    is.read(reinterpret_cast<char *>(new_bc.buckets_),
            new_bc.size() * sizeof(faded_bucket));
    bc.swap(new_bc);
    return is;
  }
};
