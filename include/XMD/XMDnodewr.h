...
//   if (clear_lock) flushed_page->typeVersionLockObsolete = 0;
...
//   if (mem_node->typeVersionLockObsolete != 0) return nullptr;
...
//       auto mem_child =
//           reinterpret_cast<NodeBase *>(inner->children[i].val &
//           swizzle_hide);
...
NodePage *recursive_iterate(NodePage *mem_node, int &idx_in_parent) {
  // First test whether this is a mininode and randomly select which node to
  // choose
  bool needRestart = false;
  uint64_t versionNode = mem_node->readLockOrRestart(needRestart);
  ...
  while (mem_node->header.bitmap != 0) {
    // auto inner = static_cast<BTreeInner<Key> *>(mem_node);
    if (parent) {
      parent->readUnlockOrRestart(versionParent, needRestart);
      if (needRestart) return nullptr;
    ...
    uint64_t hash_k = cachepush::murmur64(reinterpret_cast<uint64_t>(mem_node));
    int idx = hash_k % (mem_node->header.count + 1);
    if (idx == mem_node->header.count) {
      idx = LeftMostIdx;
    }

    auto next_idx = mem_node->header.closest_set(idx);
    if (next_idx == -1) return nullptr;
    idx_in_parent = next_idx;

    uint64_t next_node;
    if (next_idx == LeftMostIdx) {
      next_node = mem_node->left_ptr.val;
    } else {
      next_node = mem_node->values[next_idx];
    }

    mem_node->checkOrRestart(versionNode, needRestart);
    if (needRestart) return nullptr;

    mem_node = reinterpret_cast<NodePage *>(next_node & swizzle_hide);
  ...
  if ((mem_node->header.bitmap != 0) || (mem_node->header.pos_state != 2)) {
    mem_node->writeUnlock();
    return nullptr;
  }
...
bool check_limit_match(NodePage *parent, NodePage *child,
                       bool using_swizzle = false, bool verbose = false) {
  int idx = -1;
  if (using_swizzle) {
    idx = parent->findIdx(reinterpret_cast<uint64_t>(child) | swizzle_tag);
  } else {
    idx = parent->findIdx(child->header.remote_address.val);
  }
  if (verbose) std::cout << "The idx = " << idx << std::endl;
  if (idx == -1) return false;

  Key min_limit, max_limit;
  if (idx == LeftMostIdx) {
  ...
  if (verbose) {
    std::cout << "Parent min limit = " << parent->header.min_limit_
              << ", max limit = " << parent->header.max_limit_ << std::endl;
    std::cout << "Parent level = " << static_cast<int>(parent->header.level)
              << std::endl;
    std::cout << "Min limit in parent = " << min_limit
              << ", max_limit in parent = " << max_limit << std::endl;
    std::cout << "Child level = " << static_cast<int>(child->header.level)
              << std::endl;
    std::cout << "child min limit = " << child->header.min_limit_
              << ", max limit = " << child->header.max_limit_ << std::endl;
  }
  if (min_limit == child->header.min_limit_ &&
      max_limit == child->header.max_limit_) {
    return true;
  }
  return false;
}

bool new_check_limit_match(NodePage *parent, NodePage *child, unsigned idx) {
  ...
  //   Key min_limit = (idx == leftMostIdx) ? parent->header.min_limit_ :
  //   parent->keys[idx]; Key max_limit = (idx == parent->header.count && idx !=
  //   leftMostIdx) ? parent->header.max_limit_ : parent->keys[idx + 1];
...
bool check_parent_child_info(NodePage *parent, NodePage *child) {
  if (parent->header.level == 255) return true;
  bool verbose = false;
  auto parent_level = static_cast<int>(parent->header.level);
  auto child_level = static_cast<int>(child->header.level);
  if (parent_level != child_level + 1) {
    verbose = true;
  }

  if (child->header.min_limit_ < parent->header.min_limit_ ||
      child->header.max_limit_ > parent->header.max_limit_) {
    verbose = true;
  }
  ...
  if (verbose) {
    ...
    std::cout << "parent min limit = " << parent->header.min_limit_
              << "; parent max limit = " << parent->header.max_limit_
              << std::endl;
    std::cout << "child min limit = " << child->header.min_limit_
              << "; child max limit = " << child->header.max_limit_
              << std::endl;
...
void check_global_conflict(NodePage *remote_cur_node,
                           uint64_t org_version, bool &needRestart) {
  if (remote_cur_node->header.typeVersionLockObsolete != 0) {
    needRestart = true;
  }
