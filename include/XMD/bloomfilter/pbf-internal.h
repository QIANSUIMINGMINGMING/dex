...
static FORCE_INLINE uint32_t PageHash(V128X t) noexcept {
  return Rot32(t.w[0], 8) ^ Rot32(t.w[1], 6) ^ Rot32(t.w[2], 4) ^
         Rot32(t.w[3], 2);
}

template <unsigned N>
static FORCE_INLINE bool Test(const uint8_t* page, unsigned page_level,
                              V128X t) noexcept {
  ...
  if (N > 4) {
    ...
    __m256i idx =
        _mm256_and_si256(_mm256_setr_m128i(t.m, _mm_srli_epi32(t.m, 16)), mask);
    __m256i rec = _mm256_mask_i32gather_epi32(
        _mm256_set1_epi32(-1), reinterpret_cast<const int*>(page),
        _mm256_srli_epi32(idx, 5U), hole, 4);
    __m256i bit = _mm256_sllv_epi32(
        _mm256_set1_epi32(1), _mm256_and_si256(idx, _mm256_set1_epi32(31)));
...
template <unsigned N>
static FORCE_INLINE bool Set(uint8_t* page, unsigned page_level,
                             V128X t) noexcept {
  uint8_t hit = 1U;
  uint16_t mask = (1U << (page_level + 3U)) - 1U;
  for (unsigned i = 0; i < N; i++) {
