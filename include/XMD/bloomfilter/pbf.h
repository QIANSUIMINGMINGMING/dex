...
class Divisor {
 private:
  static_assert(std::is_same<Word, uint8_t>::value ||
                    std::is_same<Word, uint16_t>::value ||
                    std::is_same<Word, uint32_t>::value,
                "");
  Word m_val = 0;
#ifndef DISABLE_SOFT_DIVIDE
  static constexpr unsigned BITWIDTH = sizeof(Word) * 8;
  using DoubleWord = typename std::conditional<
      std::is_same<Word, uint8_t>::value, uint16_t,
      typename std::conditional<std::is_same<Word, uint16_t>::value, uint32_t,
                                uint64_t>::type>::type;
  using QuaterWord = typename std::conditional<
      std::is_same<Word, uint8_t>::value, uint32_t,
      typename std::conditional<std::is_same<Word, uint16_t>::value, uint64_t,
                                __uint128_t>::type>::type;
  DoubleWord m_fac = 0;
#endif
...
class _PageBloomFilter {
 ...
 protected:
  ...
  void init(unsigned page_level, unsigned page_num, size_t unique_cnt,
            const uint8_t* data);
};
...
class PageBloomFilter final : public _PageBloomFilter {
 public:
  ...
  PageBloomFilter(unsigned page_level, unsigned page_num, size_t unique_cnt = 0,
  ...
  size_t capacity() const noexcept { return data_size() * 8 / N; }
  size_t virual_capacity(float fpr) const noexcept {
    auto t = std::log1p(-std::pow(static_cast<double>(fpr), 1.0 / N)) /
             std::log1p(-1.0 / (data_size() * 8));
  ...
  static V128X partial_test1(const uint8_t* data, unsigned len, PageBloomFilter<N> *thispbf,
                           size_t & idx);

  bool partial_test2(size_t idx, V128X t) const noexcept;
};
...
extern std::unique_ptr<BloomFilter> New(unsigned way, unsigned page_level,
                                        unsigned page_num,
                                        size_t unique_cnt = 0,
                                        const uint8_t* data = nullptr);
}  // namespace pbf
