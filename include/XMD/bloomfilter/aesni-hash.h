...
static __m128i AESNI_Hash128(const uint8_t* msg, unsigned len,
                             uint32_t seed = 0) {
  ...
  bool greed =
      (((uintptr_t)msg + (len - 1)) & 0xfffUL) >= 15;  // do not cross page
...
#define GREEDILY_READ(n, addr)                                            \
  _mm_bsrli_si128(                                                        \
      _mm_bslli_si128(_mm_lddqu_si128((const __m128i*)addr), (16 - (n))), \
      (16 - (n)))

    switch (len) {
      case 15:
        mix(GREEDILY_READ(15, msg));
        break;
      case 14:
        mix(GREEDILY_READ(14, msg));
        break;
      case 13:
        mix(GREEDILY_READ(13, msg));
        break;
      case 12:
        mix(GREEDILY_READ(12, msg));
        break;
      case 11:
        mix(GREEDILY_READ(11, msg));
        break;
      case 10:
        mix(GREEDILY_READ(10, msg));
        break;
      case 9:
        mix(GREEDILY_READ(9, msg));
        break;
      case 8:
        mix((__m128i)_mm_load_sd((const double*)msg));
        break;
      case 7:
        mix(GREEDILY_READ(7, msg));
        break;
      case 6:
        mix(GREEDILY_READ(6, msg));
        break;
      case 5:
        mix(GREEDILY_READ(5, msg));
        break;
      case 4:
        mix((__m128i)_mm_load_ss((const float*)msg));
        break;
      case 3:
        mix(GREEDILY_READ(3, msg));
        break;
      case 2:
        mix(GREEDILY_READ(2, msg));
        break;
      case 1:
        mix(GREEDILY_READ(1, msg));
        break;
      case 0:
...
static inline uint64_t AESNI_Hash64(const uint8_t* msg, unsigned len,
                                    uint32_t seed = 0) {
  union {
    uint64_t x[2];
    __m128i v;
